  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <main className="min-w-0 flex-1 overflow-y-auto">
        <PageHeader eyebrow="Preferences" title="Settings" description="Control how ProdCollab watches, syncs, and notifies you." />
        <section className="m-6 max-w-2xl border border-border bg-card p-6 md:m-8">
          ...
          <label className="mt-6 flex items-center justify-between gap-4 border-b border-border pb-5 text-sm">
            ...
            <Toggle checked={autoPull} label="Automatic pull" onChange={(enabled) => { setAutoPull(enabled); window.localStorage.setItem('prodcollab_auto_pull', String(enabled)); }} />
          </label>
