    ...
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <main className="min-w-0 flex-1 overflow-y-auto">
        ...
        <section className="m-6 max-w-2xl border border-border bg-card p-6 md:m-8">
          ...
          <div className="border-b border-border py-5"><p className="text-sm">Automatic backup delay</p><p className="mt-1 text-xs text-muted-foreground">Choose how long ProdCollab waits after your last change.</p><div className="mt-3 flex flex-wrap gap-2">{['automatic', '1', '2', '5', '10', '30', 'manual'].map((delay) => <button key={delay} onClick={() => { setAutoPushDelay(delay); updatePreference('prodcollab_auto_push_delay', delay); window.electronAPI?.setAutoPushDelay(delay); window.dispatchEvent(new CustomEvent('prodcollab:auto-push-delay-changed', { detail: { delay } })); }} className={`rounded-md border px-3 py-2 text-xs transition-colors ${autoPushDelay === delay ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/40 hover:text-foreground'}`}>{delay === 'automatic' ? 'Automatic' : delay === 'manual' ? 'Manual' : `${delay} min`}</button>)}</div></div>
