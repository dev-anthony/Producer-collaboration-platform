...
export default function RemoteSession({ projectId, onClose }) {
  ...
  const create = async (nextRole, requestedId) => {
    ...
    try {
      ...
      const next = new RendererTransport({
        sessionId: nextId, role: nextRole, ...config,
  ...
  const stop = async () => {
    ...
    if (result?.path) {
      const replayUrl = await window.electronAPI.rrReadAudioFile(result.path);
      setTakes((current) => [...current, { path: result.path, replayUrl, number: takeNumber }]);
    }
  ...
  const toggleTalkback = async () => {
    if (talkbackActive) { talkbackStream.current?.getTracks().forEach((track) => track.stop()); talkbackStream.current = null; setTalkbackActive(false); return; }
    try {
      ...
      talkbackStream.current = stream; await transport.current?.addTalkbackStream(stream); setTalkbackActive(true);
