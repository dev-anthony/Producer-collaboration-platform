...
export default function RemoteSession({ projectId, onClose }) {
  ...
  const handleAudio = (data) => {
    console.log(`[RR-RECEIVER] Received PCM chunk: ${data.byteLength} bytes`);
    outputNode.current?.push(data);
    window.electronAPI.rrWriteStreamChunk?.(data);
    window.dispatchEvent(new CustomEvent('prodcollab:rr-audio-received', { detail: { bytes: data.byteLength } }));
  };
